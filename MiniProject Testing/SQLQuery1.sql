...
(
	...
	Telephone VARCHAR(15) CONSTRAINT CHK_Telephone CHECK(Telephone LIKE '[7-9][7-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]') ,
...
CREATE TABLE Policy								/* create the new Policy Table */
(
...
(
	...
	Address VARCHAR(250),
	Telephone VARCHAR(15) CONSTRAINT CHK_Telephone_Endorsement CHECK(Telephone LIKE '[7-9][7-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]'),
...
--------------------------------------------------------------------------------------------------------------------------------------------------------
/* Insert the Admin Login Credentials */ 
...
CREATE PROC prcCustomerInsert 							/* Insert Procedure */				
...
END
---------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcCustomerDetails			/* customerDeatils Procedure */
AS
BEGIN 
SELECT * FROM Customers
END

Drop proc prcCustomerID
go
------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcCustomerID			/* CustomerID Procedure */
AS
BEGIN 
SELECT max(CustomerNumber) as latestCustomer FROM Customers
END
-------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcProductInsert					/* ProductInsert Procedure */
...
CREATE PROC prcProductID									/* ProductID Procedure */
...
END
-------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcPolicyInsert								/* PolicyInsert */
						@insuredName VARCHAR(50),
						@insuredAge INT,
						@nominee VARCHAR(50),
						@relation VARCHAR(50),
...
END
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcAgeDob									/* AgeDob Procedure */ 
						@customerNumber VARCHAR(8)
AS
BEGIN
...
CREATE PROC prcLoginGen									/* LoginGeneration Procedure */
						@customerNumber VARCHAR(8),
						@loginID VARCHAR(30),
						@password VARCHAR(30)
AS
...
END
--------------------------------------------------------------------------------------------------------------------------------------------------------------
drop proc prcPolicySearchID							/* PolicySearch Procedure */
...
CREATE PROC prcPolicySearchCust								/*PolicySearchCustomer Procedure */
							@customerNumber VARCHAR(8)
AS
BEGIN
SELECT Policy.PolicyID,Products.ProductID,Products.ProductType,Products.ProductName,Policy.InsuredName,Policy.InsuredAge,Customers.Dob,
...
END
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcPolicySearchName								/* PolicySearchNAme Procedure */
							@customerName VARCHAR(50),
							@dob Date
AS
BEGIN
...
CREATE PROC prcLoginCheck										/* LoginCheck Procedure */
...
END
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcLoginDetails									/* LoginDetails Procedure*/
...
CREATE PROC prcEndorsementInsert						/* EndorsementInsert Procedure */
						@policyID VARCHAR(8),
						@productType VARCHAR(60),
						@productName VARCHAR(50),
						@insuredName VARCHAR(50),
...
END
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prctransactionID					/* TransactionID Procedure */
						@policyID VARCHAR(8)
AS
BEGIN 
...
END
----------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcStatusupdate					/* StatusUpdate Procedure */
						@transactionID INT
AS
BEGIN
...
CREATE PROC prcEndorsementDetailsCust			/* EndoresementDetailsCust Procedure */
							@customernumber VARCHAR(8)							
AS
BEGIN
...
END
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcEndorsementDetails					/* EndorsementDetails Procedure */					
AS
BEGIN
		SELECT Endorsement.TransactionID,Endorsement.PolicyID,Endorsement.InsuredName,Endorsement.InsuredAge,Endorsement.Dob,Endorsement.Gender,Endorsement.Nominee,Endorsement.Relation,
		Endorsement.Smoker,Endorsement.Address,Endorsement.Telephone,Endorsement.PremiumFrequency,EndorsementStatus.StatusID,EndorsementStatus.CurrentStatus
...
END
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcEndorsementPolicyIDDetails				/* EndoresementPolicyIDDeatils Procedure */
									@policyID VARCHAR(8)						
AS
BEGIN
		SELECT Endorsement.TransactionID,Endorsement.PolicyID,Endorsement.InsuredName,Endorsement.InsuredAge,Endorsement.Dob,Endorsement.Gender,Endorsement.Nominee,Endorsement.Relation,
...
CREATE PROC prcEndorsementUpdateDetails					/*EndorsementUpdateDetails Procedure */
						@policyID VARCHAR(8),						
						@insuredName VARCHAR(50),
						@insuredAge INT,
						@dob DATE,
...
CREATE PROC prcStatusUpdateddd									/*EndorsementStatusUpdated Procedure */
                          @transactionID INT,
...
END
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcPolicyUpdate							/* PolicyUPdate Procedure */
					  @policyID VARCHAR(8),
				      @insuredName VARCHAR(50),
					  @insuredAge INT,					  
					  @nominee VARCHAR(50),
...
END
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC prcCustomerUpdate							/* CustomerUpdate Procedure */
							@customernumber VARCHAR(8),
							@dob DATE ,
							@gender CHAR,
							 @smoker VARCHAR(5),
...
CREATE PROC prccustIDGen							/* CustIdGEn Procedures */
						@policyID VARCHAR(8)
AS
BEGIN
...
CREATE PROC prcTransactionDetails							/* TRansactionDetails Procedure */
								@transactionID INT,
								@policyID VARCHAR(8),
								@insuredName VARCHAR(50),
								@insuredAge INT,
...
CREATE PROC prcTransDetails					/* TransDetails Procedure */
AS
BEGIN
...
CREATE PROC prcTransactionDetailsID							/* TransactionDeatilsID Procedure */
					@customerNumber VARCHAR(10)
AS
BEGIN
SELECT Transactions.*
