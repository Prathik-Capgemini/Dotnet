...
namespace Capgemini.PolicyEndorsement.BusinessLayer
{
    public class PolicyBL
    {
        ...
        public static bool Validatecustomer(Customer customer)
        {
            ...
            string res = customer.CustomerName.Trim();
            if (res == string.Empty)
            {
                isValidcustomer = false;
                sb.Append("Customer Name cannot be Null" + Environment.NewLine);
            }
            string res1 = customer.Address.Trim();
            if (res1 == string.Empty)
            {
                isValidcustomer = false;
                sb.Append("Address cannot be Null" + Environment.NewLine);
            }
            if(!(Regex.IsMatch(customer.CustomerName, "[A-Z][a-z]{3,}")))
            {
                isValidcustomer = false;
                sb.Append("Name must start with uppercase" + Environment.NewLine);
            }
            if(customer.Address==string.Empty)
            {
                isValidcustomer = false;
                sb.Append(Environment.NewLine + "Please add the address");
            }
            
            if (customer.Hobbies == string.Empty)
            {
                isValidcustomer = false;
                sb.Append(Environment.NewLine + "Please add the hobbies");
            }
            if (!(Regex.IsMatch(customer.Telephone, "[7-9]{2}[0-9]{8}")))
            {
                isValidcustomer = false;
            ...
            if(customer.Gender=="X"|| customer.Smoker=="X")
                {
                isValidcustomer = false;
                sb.Append("Enter the necessary Details");
                }
